...
export function HandwrittenNote({ lines = [], to, tone = "sakura", size = 26, style, ...rest }) {
  ...
  return (
    <div style={{
      borderRadius: "var(--radius-lg)", padding: tone === "plain" ? 0 : "var(--space-6)",
      fontFamily: "var(--font-script)", fontSize: size, lineHeight: 1.4,
      ...tones[tone], ...style,
    }} {...rest}>
      ...
      <div style={{ transform: "translate(-0.115em,-0.095em)" }}>
        {all.slice(0, 3).map((l, i) => <div key={i}>{l}</div>)}
      </div>
    </div>
  );
}
